PROGRAM calc_q2
  ...
  !! $Id: calc_q2.f90 304 2011-09-15 08:20:24Z molines $
  !!----------------------------------------------------------------------------
  USE netcdf
  IMPLICIT NONE
  ...
  IF ( narg /= 2 ) THEN
     PRINT *, ' USAGE : calc_q2 filenameD filnameT'
  ...
  cfileout=cfilenameD(1:ipos-1)//'2Q'//cfilenameD(ipos+2:)
  ...
  IF ( nptD /= 12 ) THEN
     PRINT *, ' input files are both supposed to hold 12 time frames'
     PRINT *, ' This file only have ', nptD, 'time frames ...'
     PRINT *, ' Sorry :( '
     STOP
  ENDIF
  ...
  IF ( nptT /= 12 ) THEN
     PRINT *, ' input files are both supposed to hold 12 time frames'
     PRINT *, ' This file only have ', nptT, 'time frames ...'
     PRINT *, ' Sorry :( '
     STOP
  ENDIF
  ...
  istatus=NF90_INQ_VARID(ncidT,'t2',idvT2)
